...
let Config = {
	...
	logCacheSize: process.env.LOG_CACHE_SIZE
	...
	logComponents: process.env.LOG_COMPONENTS
...
module.exports.configure = function(config = {}) {
	...
	if (typeof config.logTimeDelta === "boolean") {
		Config.logTimeDelta = config.logTimeDelta;
	}

	if (typeof config.logStackTraces === "boolean") {
	...
	if (config.logFilelogFileNameAbsolute) {
		Config.logFileNameAbsolute = config.logFileNameAbsolute;
	}
		...
		if (process.stdout.isTTY) {
			...
			if (!Config.logLayout) {
				Config.logLayout = {
					...
					pattern:
						"%d{" +
						(Config.logDateFormat || "yyyy-MM-dd hh:mm:ss.SSS") +
						"} [%5.5p] - %m"
				};
			}
		}
	...
	if (Config.logFile && defaulting) {
		...
		Config.logger.categories.default.appenders.push("component");

		if (Config.logFileLayout) {
			Config.logger.appenders.component.layout = Config.logFileLayout;
		} else {
			Config.logger.appenders.component.layout = {
				type: "pattern",
				pattern:
					"%d{" +
					(Config.logDateFormat || "yyyy-MM-dd hh:mm:ss.SSS") +
